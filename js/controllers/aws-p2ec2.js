  ...
  // Runs the helloworld-runscript.sh script on the worker.
    ...
    $http({
      ...
      if( $scope.ebsvolumestatus.Volumes[0].State == "available" ) {
        ...
        $timeout( function(){$scope.Migrate_WaitForVolume(nextfn)}, $scope.delay );
      }
  ...
  // Runs the helloworld-runscript.sh script on the worker.
    ...
    $http({
      ...
      if( $scope.ebsvolumestatus.Volumes[0].Attachments[0].State != "attached" ) {
        $timeout( function(){$scope.Migrate_WaitForAttach(i+1)}, $scope.delay );
      } else {
  ...
  // Runs the helloworld-runscript.sh script on the worker.
    ...
    $http({
      ...
      if( $scope.snapshotstatus.Snapshots[0].State == "completed" ) {
        ...
        $timeout( function(){$scope.Migrate_WaitForSnapshot(nextfn)}, $scope.delay );
      }
  ...
  // Runs the helloworld-runscript.sh script on the worker.
    ...
    $http({
      ...
      if( $scope.snapshotstatus.Snapshots[0].State == "completed" ) {
        ...
        $timeout( function(){$scope.Migrate_WaitForSnapshot2(nextfn)}, $scope.delay );
      }
