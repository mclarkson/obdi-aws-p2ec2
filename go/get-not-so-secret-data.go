...
func (t *Plugin) GetRequest(args *Args, response *[]byte) error {
	...
	// A whole chain of tasks are run here rather than having many
	// individual end-points kicked off by the front end. This allows
	// the user to click the migrate button and carry on doing other
	// stuff while this runs in the background.
	...
	*/
	...
	// Wipe real secret data
	awsdata.Aws_secret_access_key = "****************"
	awsdata.Aws_obdi_worker_key = "****************"
	awsjsondata, err := json.Marshal(awsdata)
	if err != nil {
		ReturnError("Marshal error: "+err.Error(), response)
		return nil
	}

	reply := Reply{0, string(awsjsondata), SUCCESS, ""}
