...
func (t *Plugin) PostRequest(args *Args, response *[]byte) error {
	...
	*/
	...
	type AWSData struct {
		...
		Aws_obdi_worker_ip    string // E.g. 52.2.0.138
		Aws_dnsserver_ip      string // E.g. 10.0.0.5
		Aws_dnsdomain         string // E.g. mycompany.com
		Aws_gateway           string // E.g. 10.0.0.1
	}
	...
	// DNSSERVER
	env_vars_str += space + `DNSSERVER="` + awsdata.Aws_dnsserver_ip + `"`

	// DNSDOMAIN
	env_vars_str += space + `DNSDOMAIN="` + awsdata.Aws_dnsdomain + `"`

	// GATEWAY
	env_vars_str += space + `GATEWAY="` + awsdata.Aws_gateway + `"`
	...
	sa := ScriptArgs{
		...
		ScriptName: "aws-p2ec2-osedits.sh",
